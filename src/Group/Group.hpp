...
private:
    ...
    static std::vector<double> upper_weight;            //weightの合計の上限
    static std::vector<double> lower_weight;            //weightの合計の下限

public:
    static int N;                                                                                                   //グループ数
    Group(int group_id);                                                                                            //コンストラクタ
    ...
    std::vector<double> item_relation(const Item& item) const;                                                      //あるアイテムとこのグループのitem_relation
    std::vector<double> sum_item_relation(const std::vector<Item>& items) const;                                    //このグループのitem_relationの合計
    std::vector<double> sum_group_relation(const std::vector<Item>& items) const;                                   //このグループのgroup_relationの合計
    ...
    static void set_upper_and_lower(const std::vector<double>& upper, const std::vector<double>& lower);            //weightの上下限を設定
    ...
    static const std::vector<double>& get_upper();                                                                  //weightの上限を取得
    static const std::vector<double>& get_lower();                                                                  //weightの下限を取得
