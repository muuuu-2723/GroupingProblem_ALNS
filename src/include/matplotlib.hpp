...
class matplotlib {
	...
	bool open() {
		...
		send_command("fig, ax = plt.subplots(figsize=(16.0, 12.0))");
    ...
    void text(double x, double y, const char *text) const {
        fprintf(p, "ax.draw_artist(ax.text(%.17f,%.17f,\'%s\')", x, y, text);
        fprintf(p, "fig.canvas.blit(ax.bbox)\n");
		fprintf(p, "fig.canvas.flush_events()\n");
		fflush(p);
    }
